...
describe('GhostMarketTransferManager', async function () {
  ...
  it("From ETH(DataV1) to ERC721(RoyalytiV1, DataV1) Protocol, Origin fees, Royalties", async () => {
    ...
    console.log("seller token balance before: ", (await erc721V1.balanceOf(accounts1)).toString())
    console.log("buyer token balance before: ", (await erc721V1.balanceOf(accounts2)).toString())

    if (hre.network.name == 'testnet' || hre.network.name == 'testnet_nodeploy' || hre.network.name == 'hardhat') {
      console.log('matchAndTransfer on', hre.network.name);
      let tx = await matchSigner.checkDoTransfers(left.makeAsset.assetType, left.takeAsset.assetType, [200, 1], left, right, { from: accounts2, value: 300 })
      tx.wait()
      console.log("tx: ", tx)
    }
    else {
      console.log("local testnet:", hre.network.name)
      await verifyBalanceChange(accounts2, 300, async () =>			//200+6buyerFee (72back)
    ...
    console.log("seller token balance after: ", (await erc721V1.balanceOf(accounts1)).toString())
    console.log("buyer token balance after: ", (await erc721V1.balanceOf(accounts2)).toString())
